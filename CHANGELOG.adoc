...
* https://github.com/oxidecomputer/dropshot/pull/701[#701] adds a new `default_handler_task_mode` field to `ConfigDropshot` and `ServerConfig`. A server configured with `HandlerTaskMode::CancelOnDisconnect` will continue to behave as it has up to this point: a client disconnect will cancel future spawned by its request. `HandlerTaskMode::Detached` will change all handlers to always run to completion, regardless of client disconnects. `ConfigDropshot::default()` will choose `HandlerTaskMode::Detached`, which is a change in behavior if using the default configuration.
