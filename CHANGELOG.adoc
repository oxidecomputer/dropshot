...
* https://github.com/oxidecomputer/dropshot/pull/106[#106] This PR contains 2 sizeable breaking changes to the structure of request-handling
functions.
    1. Handler functions will now accept a reference to the dropshot request context as `&RequestContext` instead of an `Arc<RequestContext>`
    2. The field, https://docs.rs/dropshot/0.6.0/dropshot/struct.RequestContext.html#structfield.request[`RequestContext.request`], has been removed from the public api. To access the raw hyper request, users can specify it in their handler function's argument list as `&mut Request<Body>`
