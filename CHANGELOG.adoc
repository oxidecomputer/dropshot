...
* https://github.com/oxidecomputer/dropshot/pull/701[#701] adds a new `default_handler_disposition` field to `ConfigDropshot` and `ServerConfig`. A server configured with `HandlerDisposition::CancelOnDisconnect` will continue to behave as it has up to this point: a client disconnect will cancel future spawned by its request. `HandlerDisposition::DetachFromDisconnect` will change all handlers to always run to completion, regardless of client disconnects.
