...
* https://github.com/oxidecomputer/dropshot/pull/106[#106] The type of the field, https://docs.rs/dropshot/0.6.0/dropshot/struct.RequestContext.html#structfield.request[`RequestContext.request`], has changed from `Arc<Mutex<Request<Body>>>` to just `Request<Body>`. This simplification eliminates the need to lock and unlock the request before reading, but mutating the original hyper request will no longer be possible.
