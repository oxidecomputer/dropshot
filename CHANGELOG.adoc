...
functions.
    1. Handler functions will now accept a reference to the the `RequestContext` instead of an `Arc`
    2. The field, https://docs.rs/dropshot/0.6.0/dropshot/struct.RequestContext.html#structfield.request[`RequestContext.request`], has been removed from the public api in favor of making the raw hyper request extractable directly as `&mut Request<Body>`
