...
use clap::Parser;
use openapi_manager::{Environment, ManagedApiConfig, ManagedApis};
use openapi_manager_example_apis::*;
use openapi_manager_types::{ApiBoundary, ValidationContext, Versions};
...
pub fn environment() -> anyhow::Result<Environment> {
    ...
    let env = Environment::new(
        ...
        "openapi-manager-example/documents".into(),
    )?;
    Ok(env)
}
...
fn main() -> anyhow::Result<ExitCode> {
    let app = openapi_manager::App::parse();
    let env = environment()?;
    let apis = all_apis()?;
...
#[cfg(test)]
mod tests {
    use openapi_manager::test_util::check_apis_up_to_date;
