...
//! 4. the handler -- or anything else in request handling: routing, version
//!    policies, extractors -- panics (in either task mode): the panic
//!    propagates and the connection is aborted with no response; reported
//!    as "request handling panicked" and NOT as a client disconnection;
//! 5. the server is torn down with the request in flight -- including
...
//!    (2), NOT as a handler panic, no matter why teardown happened.
//!
//! These tests pin all five, so that changes to how panics are detected can
//! demonstrate they distinguish the cases correctly.
//!
//! The contract, in short: "request handling cancelled (client
//! disconnected)" means the client went away; "request handling panicked"
//! means the server software is broken.  The two are never conflated.
//! Within case 4 there is deliberately no separate reporting arm
//! distinguishing a handler panic from a panic elsewhere in request
//! handling: the "panic_message" property of the report is what identifies
//! the broken code, as `panic_reported_as_panic` (a handler panic) and
...
use dropshot::{
    ...
    HttpResponseOk, RequestContext, ServerBuilder, VersionPolicy, endpoint,
};
...
/// "handler panicked; propagating panic") are observable too.
...
/// Returns the "panic_message" property of the "request handling panicked"
/// log record, if any.  (`BunyanLogRecord` does not carry custom
/// properties, so this parses the raw log.)
fn logged_panic_message(log_path: &Utf8Path) -> Option<String> {
    std::fs::read_to_string(log_path)
        .unwrap()
        .lines()
        .filter_map(|line| serde_json::from_str::<serde_json::Value>(line).ok())
        .find(|record| record["msg"] == "request handling panicked")
...
async fn panic_reported_as_panic(test_name: &str, task_mode: HandlerTaskMode) {
    ...
    drop(log);
    let records =
        wait_for_log(&log_path, |r| log_has(r, "request handling panicked"));
    assert!(
        log_has(&records, "request handling panicked"),
        "expected the panic to be reported as a panic"
    );
    assert!(
...
/// Case 4, without the handler: a panic anywhere else in request handling
/// is reported exactly like a handler panic -- same log line, same aborted
/// connection.  What distinguishes the two is the panic message, which
/// names the broken code (compare the handler's "oh no, a panic!" asserted
/// in `panic_reported_as_panic`).  The version policy runs before the
/// handler is even looked up, so this is independent of the handler task
/// mode.
#[tokio::test]
...
async fn disconnect_reported_as_disconnect(
    ...
    let records = wait_for_log(&log_path, |r| {
        log_has(r, "request handling cancelled (client disconnected)")
    });
    let messages = records.iter().map(|r| &r.msg).collect::<Vec<_>>();
    assert!(
        log_has(&records, "request handling cancelled (client disconnected)"),
...
/// Case 5: tearing the server down while a request is in flight is a
...
fn teardown_during_external_panic(test_name: &str, task_mode: HandlerTaskMode) {
    ...
    let records = wait_for_log(&log_path, |r| {
        log_has(r, "request handling cancelled (client disconnected)")
            || log_has(r, "request handling panicked")
    });
    assert!(
        !log_has(&records, "request handling panicked"),
        "a request cancelled by server teardown must not be reported as a \
         handler panic (the handler never panicked); log: {:?}",
