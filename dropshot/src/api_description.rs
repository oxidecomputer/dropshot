...
pub struct ApiEndpointErrorMetadata {
    ...
    schema: fn(&mut schemars::gen::SchemaGenerator) -> schemars::schema::Schema,
}
    ...
    fn gen_openapi(&self, info: openapiv3::Info) -> openapiv3::OpenAPI {
        ...
        for (_, _, endpoint) in &self.router {
            ...
            error_names
                ...
                .insert(endpoint.error_type.identity);
        }
        ...
        for (path, method, endpoint) in &self.router {
            ...
            if let Some(code) = &endpoint.response.success {
                ...
                let ApiEndpointErrorMetadata { ref name, ref schema, identity } =
                    endpoint.error_type;
        ...
        for (_, ErrorSchema { schema, schema_name, disambiguated_name }) in
            errors
        {
            let mut content = indexmap::IndexMap::new();
            content.insert(
