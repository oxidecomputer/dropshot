...
{
    let level_drain = slog::LevelFilter(drain, Level::from(level)).ignore_res();
...
fn log_drain_for_file(
    ...
    slog::IgnoreResult<slog_json::Json<LineWriter<std::fs::File>>>,
    io::Error,
> {
    ...
    Ok(slog_bunyan::with_name(log_name_leaked, file)
        .build()
