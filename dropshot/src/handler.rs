...
/**
 * `HttpResponseOkPage<T: Serialize>` wraps the return type of a paginated
 * interface.  It denotes an HTTP 200 "OK" response whose body is generated by
 * serializing the sequence of objects and wrapping the result with a marker.
...
{
    type Body = Page<MarkerFields, ItemType>;
    const STATUS_CODE: StatusCode = StatusCode::OK;
}
...
where
    ...
    for<'a> &'a ItemType: Into<MarkerFields>,
{
    /* XXX duplicates ObjectList */
    fn from(
        mut response: HttpResponseOkPage<MarkerFields, ItemType>,
    ) -> HttpHandlerResult {
        let pag_params = response.0;
        let order = pag_params.order.unwrap_or(PaginationOrder::Ascending);
        let reverse = order == PaginationOrder::Descending;

        let next_page = {
            let items = &response.1;
            if let Some(last_item) = items.last() {
                Some(PaginationMarker::new(order, last_item.into()))
            } else {
                None
            }
        };
        ...
        HttpResponseOkPage::for_object(&page)
    }
}
