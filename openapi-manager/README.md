...
A fully working end-to-end example is available [within the Dropshot repository](https://github.com/oxidecomputer/dropshot/tree/main/openapi-manager-example). You're welcome to copy part or all of the example as desired.

## Basic usage

The OpenAPI manager consists of two crates:

- [`openapi-manager-types`](https://crates.io/crates/openapi-manager-types): Base types for the OpenAPI manager. Crates defining API traits may depend on `openapi-manager-types`.
- `openapi-manager` (this crate): The main implementation.
...
- A binary crate that integrates your API traits with `openapi-manager`, called the **integration point**.
